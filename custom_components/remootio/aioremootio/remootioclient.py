...
from .enums import State, FrameType, ActionType, EventType, ErrorCode, ErrorType, EventSource, DeviceType
...
class _TaskStore:
    """Minimal replacement for async_class.TaskStore tracking background tasks."""
    ...
    async def close(self) -> None:
        tasks = [task for task in self.__tasks if not task.done()]
        for task in tasks:
            task.cancel()
        if tasks:
...
class RemootioClient:
    ...
    @classmethod
    async def create(cls, *args, **kwargs) -> 'RemootioClient':
        """Construct a client and initialize it (establishing a connection if configured to)."""
            ...
            if self.__connection_options.connect_automatically:
                async with self.__lifecycle:
                    try:
                        ...
                        await self.__lifecycle.wait_for(lambda: self.connected or connecting_task.done())

                        if connecting_task.done() and connecting_task.exception() is not None:
    ...
    async def __handle_server_hello_frame(self, frame: ServerHelloFrame) -> NoReturn:
        ...
        self.__device_type = frame.device_type
    ...
    async def __encrypt_frame(self, frame: ActionRequestFrame) -> EncryptedFrame:
        ...
        try:
            ...
            # Encrypt frame (AES-256-CBC with PKCS7 padding)
            iv_bytes: bytes = os.urandom(16)
            encryptor = Cipher(algorithms.AES(bytes(session_key)), modes.CBC(iv_bytes)).encryptor()
            ...
            payload_bytes = encryptor.update(payload_bytes) + encryptor.finalize()
            ...
            mac_bytes: bytes = hmac.new(api_auth_key, data_bytes, hashlib.sha256).digest()
    ...
    async def __decrypt_frame(self, frame: EncryptedFrame) -> Union[ActionResponseFrame, EventFrame, ChallengeFrame]:
        ...
        try:
            ...
            # Verify mac (HMAC-SHA256, constant-time comparison)
            calculated_mac: bytes = hmac.new(api_auth_key, bytes(data), hashlib.sha256).digest()
            if not hmac.compare_digest(frame.mac, str(b64encode(calculated_mac), ENCODING)):
                raise RemootioClientDecryptionError(
                    self, "Frame can't be decrypted because mac verification was failed.")

            # Decrypt frame (AES-256-CBC with PKCS7 padding)
            decryptor = Cipher(algorithms.AES(bytes(session_key)), modes.CBC(iv)).decryptor()
            payload_bytes = decryptor.update(payload_bytes) + decryptor.finalize()
            unpadder = crypto_padding.PKCS7(algorithms.AES.block_size).unpadder()
            payload_bytes = unpadder.update(payload_bytes) + unpadder.finalize()
    ...
    def device_type(self) -> Optional[DeviceType]:
        """
        :return: Model/type of the device as reported in its SERVER_HELLO frame
        """
