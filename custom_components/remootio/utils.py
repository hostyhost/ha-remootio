...
async def get_serial_number(
    ...
    async with asyncio.timeout(REMOOTIO_TIMEOUT):
        remootio_client: RemootioClient = await RemootioClient.create(
            connection_options,
...
async def create_client(
    ...
    """Create a Remootio client based on the given data."""
    async with asyncio.timeout(REMOOTIO_TIMEOUT):
        result: RemootioClient = await RemootioClient.create(
            connection_options,
            aiohttp_client.async_get_clientsession(hass),
            LoggerConfiguration(logger=logger),
        )
            ...
            if expected_serial_number is not None:
                ...
                if expected_serial_number != serial_number:
                    ...
                    raise ConfigEntryError(
                        "Serial number of the Remootio device isn't the expected one"
                    )
